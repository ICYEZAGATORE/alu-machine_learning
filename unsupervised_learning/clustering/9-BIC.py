...
def BIC(X, kmin=1, kmax=None, iterations=1000, tol=1e-5, verbose=False):
    ...
    Args:
        ...
        iterations: positive integer, maximum EM iterations
        tol: non-negative float, EM tolerance for early stopping
        verbose: boolean, whether EM prints log likelihood info
    ...
    log_l = np.array(likelihoods)
    b = np.array(bics)
    best_idx = np.argmin(b)
    best_k = kmin + best_idx
    best_result = results[best_idx]

    return best_k, best_result, log_l, b
